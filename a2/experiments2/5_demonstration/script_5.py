...
def methoddef(name, color, model, optimizer,
  ...
  method.meas = [meas.meas_iter(epoch, "step"),
                 meas.meas_rnnloss(model.x, model.y,
                                   ...
                                   model.final_state,
                                   data_train, model.show_train_loss, "train_loss",
                                   BATCH, STEPS,
                                   axes=[0.0, np.inf, 0.0, YMAX_TRAIN]),
                 meas.meas_rnnloss(model.x, model.y,
                                   model.zero_state, model.init_state,
                                   model.final_state,
                                   data_train, model.misclass_err, "train_err",
                                   BATCH, STEPS,
                                   axes=[0.0, np.inf, 0.0, YMAX_TRAIN]),
                 meas.meas_rnnloss(model.x, model.y,
                                   model.zero_state, model.init_state,
                 ...
                 meas.meas_rnnloss(model.x, model.y,
                                   model.zero_state, model.init_state,
                                   model.final_state,
                                   data_valid, model.misclass_err, "valid_err",
                                   BATCH, STEPS,
                                   axes=[0.0, np.inf, 0.0, YMAX_VALID]),
                 meas.meas_rnnloss(model.x, model.y,
                                   model.zero_state, model.init_state,
                                   model.final_state,
                                   data_test, model.show_train_loss, "test_loss",
                                   BATCH, STEPS,
                                   axes=[0.0, np.inf, 0.0, YMAX_VALID]),
                 meas.meas_rnnloss(model.x, model.y,
                                   model.zero_state, model.init_state,
                                   model.final_state,
                                   data_test, model.misclass_err, "test_err",
                                   BATCH, STEPS,
                                   axes=[0.0, np.inf, 0.0, YMAX_VALID]),
...
gate_fun = tf.nn.relu
loss_fun = lambda z_hat, y: util2.losses.sparse_kl_divergence_ml(z_hat, y, tau=0.2)
...
pickle.dump( [means, out_dir, FILELABEL, FILETAG], open( "save.p", "wb" ) )
