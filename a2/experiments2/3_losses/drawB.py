...
gate_fun = tf.nn.tanh
loss_fun = lambda z_hat, y: util2.losses.sparse_kl_divergence_ml(z_hat, y, tau=0.1)
...
cell_type = util2.rnn_simple.SimpleLSTMCell
gate_fun = tf.nn.tanh
loss_fun = lambda z_hat, y: util2.losses.sparse_kl_divergence_rl(z_hat, y, tau=0.5)
dimensions = hidden, depth, vocab
model = model_rnn(name, cell_type, dimensions, gate_fun, loss_fun)
  # optimizer
init_step = 10.
...
means, out_dir, FILELABEL, FILETAG = pickle.load( open( "saveB.p", "rb" ) )
